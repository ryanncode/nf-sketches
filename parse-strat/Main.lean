...
def formatDetailedCycleSandbox (c : List Var) (edges : List Edge) : String :=
  ...
  if pairs.length >= 2 then
    ...
    let req1 := s!"l({varToString endVar}) = l({varToString startVar}) + {forwardWeight}"
    let req2 := if backWeight == 0 then s!"l({varToString endVar}) = l({varToString startVar})"
                else s!"l({varToString endVar}) = l({varToString startVar}) + {-backWeight}"
