...
* **Lexical Scanner & Parser**: Converts raw user input strings into Abstract Syntax Trees (AST).
...
### Syntax Cheatsheet
Before diving into proofs, here is exactly how the `parse-strat` Lexical Scanner reads standard mathematical syntax:

* **Variables & Predicates:** `x`, `y`, `P(x)`, `Q(a,b)`
* **Logical Connectives:**
    * AND: `/\` (e.g., `A /\ B`)
    * OR: `\/` (e.g., `A \/ B`)
    * IMPLIES: `->` (e.g., `A -> B`)
...
* `cut <formula>`: Interactive diagnostic cut tactic. Deliberately introduces a complex/saturated topology formula into the context, testing for Extensionality bounds.
