...
Bellman-Ford witness generated by `NfValidate`). The `reduceCut` engine operationalizes
Marcel Crabbé's cut-elimination results, demonstrating exactly how structural normalization
breaks down when cuts interact with stratified comprehension.
-/
...
inductive Derivation : Sequent → Type
  ...
  those formulas that are grammatically valid.
  -/
...
mathematically demonstrating the normalization breakdown of systemic ambiguity.
-/
