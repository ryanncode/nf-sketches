...
This strategy depends on collapsing harmless 0-weight semantic cycles. The system implements Kosaraju's algorithm via a single-pass `findSCCs` and `flattenGraph` to identify these 0-weight equivalence bounds (Strongly Connected Components) efficiently and flatten identical-level variables. We support this with utilities like `freshVar` and `expandConnectives` to securely generate isolated variables and standardize boolean operator networks. Additional constructs—including `isPair`, `isProj1`, `acyclicEq`, and `acyclicV`—synthesize acyclic nodes to build Quine ordered pairs and structural relations without injecting artificial cycles into the AST.
...
For manual experimentation, the `parse-strat` module provides an interactive REPL sandbox. A recursive descent parser translates raw ASCII via `tokenize` and `parseFormula` directly into `Formula` AST nodes, which is seamlessly piped through a monolithic tactic ecosystem (`intro`, `rewrite`, `simp`, `collapse_loop`) forming a robust AST-to-DAG compilation pipeline. Diagnostic formatters like `formatWitness` and `formatDetailedCycleSandbox` map theoretical negative-weight loops backward into plain language algebraic contradiction paths.
...
1. **Lazy Thunks & Suspensions:** The combinatory AST integrates explicit `lazy_thunk` wrappers acting as Okasaki Suspensions, alongside a `terminal` state constructor.
2. **Karp's Minimum Cycle Mean (MCM):** The engine tracks semantic topological friction through Karp's MCM algorithmic approximation. By extracting the minimum average edge weight over isolated 0-weight semantic cycles, the compiler generates a dynamic graph constraint, identifying the exact negative-weight boundaries where computation collapses into paradoxical regression ($\mu^* < 0$).
3. **Interaction Monoids & Girard's Formula:** The system formally incorporates Mazza's Interaction Monoids endomorphisms. Girard's formula (`execute_path_weight`) algebraically traces the execution geometry of topological loops to track structural permutations directly as numerical limits.
4. **K-Iteration Halting:** These unified limits dictate the execution horizon. The engine natively intercepts self-referencing evaluations and freezes the result into the AST as `Comb.terminal "K_ITERATION_HALT"`.
...
- **Physical Observables:** `Observables.lean` explicitly extracts theoretical physics metrics (Rest Mass, Coupling Constant, Resonance Magnitude) directly from measuring the bounded topological recursion cost calculated by the evaluator.
...
8. **`CausalIrreducibility.lean`**: Validates discrete Causal Graph Syntactic Monism by simulating a symmetric, paradox-laden topological boundary, safely measuring algorithmic randomness as structural recursion bounds.
9. **`ColliderSimulation.lean`**: Physically proves that tracking dynamic extensional collisions inside a finite graph requires tracking topological recursion bounds. It calibrates a dimensionless Resonance Magnitude ($T_c / T_0$) from a high-energy structural intersection and safely resolves the unbounded recursion cascade via Strongly Cantorian Fixpoint calculation.
10. **`AgenticReflection.lean`**: Implements a functional, self-referential autonomous agent operating through natively sandbox-insulated reflection loops, utilizing Minimum Cycle Mean recursion bounds to determine structural decision-making and logging results via Knaster-Tarski stability checks.
11. **`AgenticPlanning.lean`**: Expands agentic reflection into a parallel decision matrix, structurally evaluating divergent topologies simultaneously and mechanically selecting the outcome requiring the lowest recursive topological cost (Principle of Least Syntactic Action).
