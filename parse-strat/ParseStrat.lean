...
def formatWitness (w : List (ScopedVar × Int)) : String :=
  let pairs := w.map (fun (v, l) => s!"{scopedVarToString v} : {l}")
...
def formatDetailedCycleSandbox (c : List ScopedVar) (edges : List Edge) : String :=
  ...
  let pathStrings := pairs.map (fun (src, dst) =>
    ...
    else s!"{scopedVarToString dst} ∈ {scopedVarToString src} (-1)" -- Handles reverse edges if they appear in the cycle
  )
