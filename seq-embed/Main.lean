...
Crucially, this module evaluates **weak stratification**. By partitioning the logical formula
into disjoint subgraphs based on quantifier scope, we allow local stratifiability without
forcing a monolithic global type assignment. The `reduceCut` engine operationalizes Marcel
Crabbé's observation that restricting a system to stratifiable terms destroys closure under
substitution. It demonstrates exactly how structural normalization breaks down via
grammatical substitution collisions when cuts interact with even weakly stratified comprehension.
-/
...
/--
Syntactic sugar for biconditional implication ($p \leftrightarrow q$).
-/
def mkIff (p q : Formula) : Formula :=
  Formula.conj (Formula.impl p q) (Formula.impl q p)

/--
Syntactic sugar for existential quantification ($\exists n. p$).
-/
def mkExists (n : String) (p : Formula) : Formula :=
  Formula.neg (Formula.univ 0 n (Formula.neg p))

/--
Constructs the NF Comprehension Axiom body: $\exists x \forall y (y \in x \leftrightarrow \phi)$
Using a locally nameless representation, inside $\forall y$, $y$ is `bound 0` and $x$ is `bound 1`.
-/
...
/--
Constructs the Extensionality Axiom: $\forall x \forall y (\forall z (z \in x \leftrightarrow z \in y) \to x = y)$
Using a locally nameless representation, inside $\forall z$, $z$ is `bound 0`, $y$ is `bound 1`, $x$ is `bound 2`.
-/
...
inductive Derivation : Sequent → Type
  ...
  those formulas that satisfy the weak stratification boundaries of NF.
  -/
...
By forcing impredicative substitutions into a weakly stratified graph,
...
/-- 1. The Identity Collapse: Cut on $z=A$ against `compL` on $\forall y(y \in A \leftrightarrow y = z)$ -/
...
/-- 2. The Impredicative Singleton: Cut on $w=S$ against `compL` on $\forall x(x \in S \leftrightarrow x \notin w)$ -/
...
/-- 3. The Transitive Membership Collapse: Cut on $A=C$ against `compL` on $\exists A \forall y(y \in A \leftrightarrow y \in B \land B \in C)$ -/
...
/-- 4. The Russell-Prawitz Normalization Breakdown: Cut on $x=R$ against `compL` on $\exists R \forall x(x \in R \leftrightarrow x \notin x)$ -/
...
def runDiagnostic (testName : String) {Γ Δ : Context} (A : Formula) (d1 : Derivation ⟨Γ, A :: Δ⟩) (d2 : Derivation ⟨A :: Γ, Δ⟩) : IO Unit := do
  ...
  | Except.error (ReductionError.StratificationFailure msg cycle edges) =>
      ...
      IO.println (formatDetailedCycle cycle edges)
