...
This strategy depends on collapsing harmless 0-weight semantic cycles. The system implements Tarjan's algorithm (upgraded from Kosaraju's) via a single-pass `findSCCs` and `flattenGraph` to identify these Strongly Connected Components (SCCs) efficiently and flatten identical-level variables. We support this with utilities like `freshVar` and `expandConnectives` to securely generate isolated variables and standardize boolean operator networks. Additional constructs—including `isPair`, `isProj1`, `acyclicEq`, and `acyclicV`—synthesize acyclic nodes to build Quine ordered pairs and structural relations without injecting artificial cycles into the AST.
